...
interface AdminUser {
  ...
  fullName: string;
  email: string;
  role: "user" | "donor" | "campaigner" | "admin" | "super-admin";
...
const roleLabels: Record<string, string> = {
  user: "Individual",
  donor: "Donor",
  campaigner: "Campaign Manager",
  admin: "Admin",
  "super-admin": "Admin"
};

// Normalization function: combine firstName + lastName if fullName not present
const normalizeUser = (user: any): AdminUser => ({
  ...
  organizationName: user.organizationName ?? user.org_name ?? "N/A",
  createdAt: user.createdAt ?? user.created_at ?? null,
  campaignCount: user.campaignCount ?? user.campaign_count ?? 0,
  lastLogin: user.lastLogin ?? user.last_login ?? null,
  totalDonations: user.totalDonations ?? user.total_donations ?? 0,
  notes: user.notes ?? "",
});
...
export default function UserManagementPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      console.log("[fetchUsers] API raw response:", data);
  ...
  const fetchSingleUser = async (userId: string) => {
    ...
    try {
      ...
      console.log("[fetchSingleUser] Status:", res.status);

      if (res.status === 403) {
        toast.error("Forbidden: Cannot view this user");
        return;
      }

      const data = await res.json();
      console.log("[fetchSingleUser] Response data:", data);

      setViewUser(normalizeUser(data));
      setViewDialogOpen(true);
    } catch (err) {
      console.error("[fetchSingleUser] Error:", err);
      toast.error("Failed to load user details — check console for details.");
    }
  };
  ...
  const handleUserAction = async () => {
    ...
    const token = localStorage.getItem("superAdminToken");
    if (!token) {
      toast.error("Super Admin token not found. Please log in again.");
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <Link href="/dashboard" className="text-sm text-red-600 hover:underline inline-flex items-center gap-2">
            ...
            <div className="w-full md:w-40">
              ...
              <Select value={roleFilter} onValueChange={setRoleFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="campaigner">Campaign Manager</SelectItem>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  <p>Role: {roleLabels[user.role] || user.role}</p>
                  <p>Organization Name: {user.organizationName || "N/A"}</p>
                  <p>Campaigns: {user.campaignCount ?? 0}</p>
                  <p>Total Donations: ${(user.totalDonations ?? 0).toLocaleString()}</p>
                  <p>Last Login: {user.lastLogin ? new Date(user.lastLogin).toLocaleDateString("en-GB") : "N/A"}</p>
                  <p>Registered: {user.createdAt ? new Date(user.createdAt).toLocaleDateString("en-GB") : "N/A"}</p>
                </div>
              </div>
        ...
        <DialogContent>
          ...
          {viewUser ? (
            <div className="space-y-2 text-sm text-gray-700">
              ...
              <p><strong>Role:</strong> {roleLabels[viewUser.role] || viewUser.role}</p>
              <p><strong>Status:</strong> {viewUser.status}</p>
              <p><strong>Registration Status:</strong> {viewUser.registrationStatus}</p>
              <p><strong>Organization:</strong> {viewUser.organizationName || "N/A"}</p>
              <p><strong>Campaigns:</strong> {viewUser.campaignCount ?? 0}</p>
              <p><strong>Total Donations:</strong> ${(viewUser.totalDonations ?? 0).toLocaleString()}</p>
              <p><strong>Last Login:</strong> {viewUser.lastLogin ? new Date(viewUser.lastLogin).toLocaleDateString("en-GB") : "N/A"}</p>
              <p><strong>Created:</strong> {viewUser.createdAt ? new Date(viewUser.createdAt).toLocaleDateString("en-GB") : "N/A"}</p>
            </div>
          ) : (
...
//                 {user.status === "active" && (
//                   <>
//                     <Button variant="outline" size="sm" onClick={() => handleAction(user, "pause")}>
//                       <Pause className="w-4 h-4" />
//                     </Button>
//                     <Button variant="outline" size="sm" onClick={() => handleAction(user, "delete")}>
...
//                 {user.status === "paused" && (
//                   <>
//                     <Button variant="outline" size="sm" onClick={() => handleAction(user, "activate")}>
...
//                 {user.status === "deleted" && (
//                   <Button variant="outline" size="sm" onClick={() => handleAction(user, "restore")}>
//                     <Shield className="w-4 h-4" />
//                   </Button>
//                 )}

//                 {/* Always show Eye */}
//                 <Button variant="outline" size="sm" onClick={() => fetchSingleUser(user.id)}>
